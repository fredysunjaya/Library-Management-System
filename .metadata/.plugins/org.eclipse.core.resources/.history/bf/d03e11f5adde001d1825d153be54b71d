...
public class AdminIssuedFrame extends JFrame implements ActionListener{
	...
	public void loadTable(ArrayList<IssuedBook> issuedBooks, boolean type) {
...
//	}
	...
	public void initComponent(ArrayList<IssuedBook> issuedBooks, boolean type) {
	...
	public AdminIssuedFrame(Library library, int width, int height, Point point, boolean type) {
		...
		initComponent(library.getIssuedBooks(), type);
...
//			}
			...
			if(!searchValue.equals("")) {
				...
				ArrayList<IssuedBook> searchedRecords = library.filterRecords(searchValue);
			...
			if(selectedRow != -1) {
				...
				if(answer == JOptionPane.YES_OPTION) {
					...
					library.changeIssuedBook(library.getIssuedBooks().get(selectedRow));
					issuedTable.invalidate();				
				}
			} else {
